...
test_that("import_aeron fails to import data", {
  ...
  expect_error(import_aeron(aeronfile, "Invalid-genome"))
  expect_error(import_aeron(aeronfile, "hg19", "invalid limit"))
})

test_that("import_aeron limits work", {
  expect_equal(length(import_aeron(aeronfile, "hg19", 2)), 2)
})
